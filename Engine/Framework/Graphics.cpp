...
Graphics::Graphics( HWNDKey& key , unsigned int width,unsigned int height )
	:
	ScreenWidth(width),
	ScreenHeight(height),
	sysBuffer(width, height)
{
...
void Graphics::DrawSprite(int dstX, int dstY, const Surface & surf)
{
	int width = surf.GetWidth();
	int height = surf.GetHeight();
	const Color* surff = surf.GetBufferPtrConst();
	const int yStart = std::max(-dstY, 0);
	const int xStart = std::max(-dstX, 0);
	const int yEnd = std::min((int)ScreenHeight - dstY, height);
	const int xEnd = std::min((int)ScreenWidth - dstX, width);
	for (int a = yStart; a < yEnd; a++) {
		for (int b = xStart; b < xEnd; b++) {
			sysBuffer.PutPixel(dstX + b, dstY + a, surf.GetPixel(b,a));
		}
	}
}

void Graphics::DrawSpriteAlfa(int dstX, int dstY, const Surface & surf)
{
	int width = surf.GetWidth();
	int height = surf.GetHeight();
	const int yStart = std::max(-dstY, 0);
